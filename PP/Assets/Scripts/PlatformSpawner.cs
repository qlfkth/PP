...
public class PlatformSpawner : MonoBehaviour
{
    ...
    public float xPos = 5f; //배치할 위치의 x 값
    ...
    void Start()
    {
        ...
        //count만크 ㅁ루프하면서 발판 생성
        for (int i = 0; i < count; i++)
        {
            //plarformPrefab을 원본으로 새 발판을 poolPosition 위치에 복제 생성
    ...
    void Update()
    {
        ...
        if (Time.time >= lastSpawnTime + timeBetSpawn)
        {
            ...
            timeBetSpawn = Random.Range(timeBetSpawnMin, timeBetSpwanMax);

            //배치할 위치의 높이를 yMin과 yMax 사이에서 랜덤 설정
            float yPos = Random.Range(yMin, yMax);

            //사용할 현재 순번에 발판 게임 오브젝트를 비활성화하고 즉시 다시 활성화
